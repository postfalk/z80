...
from subprocess import (
    check_output, CalledProcessError, Popen, PIPE)
import binascii
...
def get_command_line_options():
    ...
    ret = {}
    try:
       opts, args = getopt.getopt(argv,"hi:w",["ifile=", "help", "write"])
...
def compile(inputfile_name, binfile_name=None):
    ...
    p = Popen(["z80asm", inputfile_name, "-o", binfile_name], stdout=PIPE, stderr=PIPE)
    res = p.communicate()
    if res[0] or res[1]:
        if 'unable to open' in res[1]:
            print '\nUnable to open file.\n'
        else:
            print '\n%s' % str(res)
...
def create_arduino_code(binfile_name, outfile_name=OUTPUTFILE_NAME,
    ...
    print 'Arduino C array: %s' % outfile_name
    return outfile_name
...
def create_rom_file(bin_filename, rom_size=ROM_SIZE):
    ...
    with open(rom_filename, 'wb') as f:
        f.write(content)
        print 'EEPROM image file: %s' % rom_filename
